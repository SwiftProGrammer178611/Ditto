...
app.message(async ({ message, client }) => {
    ...
    if (isSelfDm && message.text && message.text.startsWith("!")) {
        if (message.text === "!ping") {
        ...
        }else if(message.text.startsWith("!mirror ")){
            const toMirror = message.text.slice("!mirror ".length);
            const reverse = toMirror.split("").reverse().join("");
            await client.chat.postMessage({channel:message.channel, text: reverse});
        }
        else if(message.text === "!joke"){
            try{
                const res = await fetch("https://official-joke-api.appspot.com/random_joke");
                const data = await res.json();
                await client.chat.postMessage({channel: message.channel, text: `${data.setup}\n${data.punchline}`});
            } catch(err){
                await client.chat.postMessage({channel: message.channel, text: "couldn't fetch your joke but here's one: Why do you tell actors to 'break a leg?' BECAUSE EVERY PLAY HAS A CAST 🤣"})
            }
            ...
            const city = message.text.slice("!weather ".length);
            try{
                const res = await fetch(`https://wttr.in/${encodeURIComponent(city)}?format=3`);
                const text = await res.text();
                await client.chat.postMessage({channel:message.channel, text: text});

            }catch(err){
                await client.chat.postMessage({channel:message.channel, text: "The weather service itself is itself under the weather :() "});
            }
